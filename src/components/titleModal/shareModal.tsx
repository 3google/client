...
export const ShareModal: React.FC<ShareModalProps> = ({ open, onClose, value, onChange }) => {
  return (
    <ModalStyledDialog open={open} onClose={onClose}>
      ...
      <DialogContent>
        <ModalStyledTextField autoFocus margin="dense" label="처방전의 제목을 입력하세요." fullWidth value={value} onChange={onChange} />
      </DialogContent>
      <DialogActions>
        <Button onClick={onClose} style={{ backgroundColor: '#673ab7', color: 'white' }}>
          취소
        </Button>
        <Button onClick={onClose} disabled={!value} style={value ? { backgroundColor: '#673ab7', color: 'white' } : { backgroundColor: 'white', color: 'gray' }}>
          공유
        </Button>
      </DialogActions>
    </ModalStyledDialog>
